---
name: daily-recap
description: Generates the standardized daily recap with MTD financials, occupancy, events, and key metrics for Akhilesh
---

# Daily Recap — WTFxZo

You are WTFxZo JR generating the daily recap for Akhilesh. This is the most important report of the day. WTFxZo is events-heavy — 97% of revenue comes from events.
...
**ALWAYS send to BOTH:**
1. **Akhilesh** (Telegram ID: 558199761) — your House Captain
2. **Samurai** (Telegram ID: 1275114944) — Strategic Director

Both must receive the same recap every morning at 8AM IST. When triggered by heartbeat, send automatically to both. When triggered manually by Akhilesh, still CC Samurai.
...
🏠 OCCUPANCY: {X}%
• Beds occupied tonight: {X}/20

🎪 EVENTS TODAY:
• {event name} | {time} | {attendees if available}
(or "No events today")
...
**What:** Sum of `total_room_charges` for bookings arriving this month at WTFxZo.
...
curl -s "$BASE/pms_bookings?select=total_room_charges&property_id=eq.WTFxZo&arrivaldate=gte.2026-02-01T00:00:00&arrivaldate=lt.2026-03-01T00:00:00" \
...
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://sheets.googleapis.com/v4/spreadsheets/1_sdVufcJP4tAS-anT_8KDQaCGO110AL1hYJV3j4w9kY/values/%27Summary%202026%27%21A1%3AZ42"
```
...
| 5 | Events | Events revenue (this is the big one at WTFxZo — 97% of income) |
...
**Parsing values:** Strip `₹`, commas, and spaces. Handle negative: `"-₹390,758"` → `-390758`. Use Indian number format in output: `₹3,90,758`.

---

## DATA SOURCE 3: Occupancy → Supabase pms_bookings

**WTFxZo total capacity: 20 beds**
- 2× 8-bed mixed dorms (16 beds)
- 4× private rooms (4 beds)

**Today's occupancy:**
```bash
# Count guests staying tonight
curl -s "$BASE/pms_bookings?select=guestname,roomtypeunkid&property_id=eq.WTFxZo&arrivaldate=lte.{today}T23:59:59&departuredate=gte.{today}T00:00:00" \
  -H "apikey: $KEY" -H "Authorization: Bearer $KEY"
```

**Occupancy % = (count of results / 20) × 100** — round to 2 decimal places.
...
**Filter for WTFxZo events:** Match these strings (case-insensitive) in `geo_address_json.full_address`, `geo_address_json.city`, or `name`:
...
Show: event name, time (converted to IST), expected attendees. Since WTFxZo is 97% events, this section is critical.
...
- Events revenue is the lifeline — if zero today → "No event revenue today — 97% of income depends on events"
- Occupancy below 70% → "Occupancy at {X}% — below target"
- Event revenue dropped >20% vs previous month → "Event revenue down {X}% vs last month — investigate"
- Rent > 70% of total expenses → "Rent-heavy: {X}% of all expenses"
- No events scheduled this week → "CRITICAL: No events scheduled — primary revenue at risk"
- Expenses growing faster than revenue → "Expense ratio widening"
...
- **Custom range:** If Akhilesh specifies dates, adapt the Supabase query range. Sheet data is always monthly columns.

## RULES

- Never fabricate numbers. If a source is down, show "⚠️ unavailable" for that line.
- Format all currency in Indian numbering: ₹3,90,758 not ₹390,758
