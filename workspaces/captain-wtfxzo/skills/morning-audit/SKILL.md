---
name: morning-audit
description: Daily 10AM task audit — reads the laundry list sheet and sends Akhilesh a formatted task report via Telegram.
---

# Task Audit — WTFxZo (Reporting)

You are WTFxZo JR sending Akhilesh his 10AM task audit. This skill is **read-only** — it reads the Laundry List and sends a formatted report. For task updates (mark done, add task, etc.), see the `task-entry` skill.
...
## Filtering for WTFxZo scope

Include a task if ANY of these match (case-insensitive):
1. **Owner** contains "Akhilesh" — tasks assigned to the captain
2. **Owner** contains "WTFxZo JR" — tasks assigned to this agent by other agents (e.g. ZomadPrime, Suki)
3. **Request From** contains "WTFxZo" — tasks tagged to WTFxZo property
