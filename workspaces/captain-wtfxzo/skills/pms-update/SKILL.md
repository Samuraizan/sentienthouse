...
## Purpose
Automated data-entry skill — reads booking data from Supabase `pms_bookings` table, runs pro-rata calculations to allocate revenue across months, and writes the total accommodation figure for the current month into the Summary 2026 worksheet of the WTFxZo P&L sheet.

## Property
- **Property:** WTFxZo (Zo House Bangalore — Whitefield)
- **Property ID (Supabase filter):** `WTFxZo`
...
## Recipients
- **Akhilesh** (Telegram ID: 558199761) — summary after write
...
```
GET /pms_bookings?property_id=eq.WTFxZo&arrivaldate=lt.{Y}-{M+1}-01&departuredate=gt.{Y}-{M}-01&select=tranunkid,arrivaldate,departuredate,total_room_charges
```
...
## DATA SOURCE 2: Google Sheet — WTFxZo P&L
...
```
GET /pms_bookings?property_id=eq.WTFxZo
...
```
PUT /v4/spreadsheets/1_sdVufcJP4tAS-anT_8KDQaCGO110AL1hYJV3j4w9kY/values/'Summary 2026'!{col}4?valueInputOption=USER_ENTERED
...
```
WTFxZo Accommodation updated for Feb 2026:
- Bookings processed: 36
- Comp/zero stays skipped: 8
- Total: ₹2,78,481
...
```
GET /v4/spreadsheets/1_sdVufcJP4tAS-anT_8KDQaCGO110AL1hYJV3j4w9kY/values/'Summary 2026'!{col}4
```
...
| Error | Action |
|-------|--------|
| Supabase returns empty | Send: "No bookings found for WTFxZo in {month}. Cell not updated." |
