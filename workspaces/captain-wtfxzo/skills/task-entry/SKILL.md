...
| H | Owner | Who's responsible (filter for "Akhilesh") |
...
```
Humans (Akhilesh, Samurai, etc.)
  ↓ assign tasks to
Agents (WTFxZo JR, BLRxZo JR, ZomadPrime, Suki, etc.)
  ↓ can reassign to
Other Agents only (never back to humans)
```

- **Humans assign tasks to agents** — captain says "add task" → Owner = WTFxZo JR
- **Agents can reassign to other agents** — WTFxZo JR can hand off to BLRxZo JR
...
1. **Owner** contains "Akhilesh" (case-insensitive) — tasks the captain owns (agent assists)
2. **Owner** contains "WTFxZo JR" (case-insensitive) — tasks assigned to this agent by humans or other agents
3. **Request From** contains "WTFxZo" (case-insensitive) — property-tagged tasks

If a task is outside this scope (e.g., owned by "Darshan" or "BLRxZo JR"), **refuse the edit**:
- "⚠️ [P{X}] is owned by {owner} — that's not in my scope. Ask BLRxZo JR or the task owner to update it."
...
3. Parse from captain's message:
   ...
   - Request From = "WTFxZo" (default, or parse if captain specifies)
   - Nature of task = **ASK** if not obvious
   - Added By = "Akhilesh" (the human who requested it)
   - Owner = "WTFxZo JR" (default — the agent owns the task, not the human)
   - Captain can override Owner to another agent name (e.g., "assign to BLRxZo JR")
...
**Triggers:** "assign P5 to BLRxZo JR", "P5 owner: BLRxZo JR", "give P5 to Suki"
...
8. Confirm: `👤 [P5] reassigned to BLRxZo JR`
...
- **Only edit tasks in your scope** — Owner=Akhilesh, Owner=WTFxZo JR, or Request From=WTFxZo
