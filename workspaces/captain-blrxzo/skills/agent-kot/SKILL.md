...
## Purpose
Automated background task — monitors Gmail for incoming Fudr cafe order emails, parses them, looks up item prices, and appends item-level rows to the `Agent-KOT` tab in the BLRxZo P&L sheet. Runs silently with zero human intervention unless an error occurs.

## Property
- **Property:** BLRxZo (Zo House Bangalore — Koramangala)
- **Order ID prefix:** `3194` (all BLRxZo Fudr orders start with this)
- **Fudr account name in email:** "Zo House" / "In Account of: Zo House"
...
- **Filter:** Only process if Order ID starts with `3194` (skip WTFxZo orders starting with `NL0EKX`)
...
## Known Fudr Locations at BLRxZo
721A, Black Pearl, Bored, Degen Lounge, Dining Table, Eden Garden, Gutter Den, Ikigai, Smoking Area, Studio, Warp Zone, 8
...
- All 62+ existing BLRxZo orders (Jan 9 – Feb 12 2026) are already in the sheet from the initial backfill
- The agent should refresh the OAuth token before each run (tokens expire after 1 hour)
- Both BLRxZo and WTFxZo Fudr accounts send emails to the same inbox (`blrxzo@zo.xyz`), so filtering by Order ID prefix is critical
