...
## Purpose
Data entry skill — captain sends an image (invoice/receipt/screenshot) or text message about an expense, agent parses, confirms with captain, and appends a row to the Running OPEX tab in the BLRxZo P&L sheet.
...
Body: {"values": [["{date}", "{month}", "{item}", "{quantity}", "{head_of_account}", "{amount}", "{payment_mode}", "{remarks}"]]}
```
...
| Month | Derive from the Date field | Current month (format: `Mon-YYYY`, e.g. `Feb-2026`) |
| Item | Description of what was purchased/paid for | **ASK** — do not guess |
...
**CRITICAL: Never guess Item, Head of Account, or Amount. If you can't parse them from the image/text, ASK the captain.**
...
• Date: 12/02/2026
• Month: Feb-2026
• Item: Kitchen gas cylinder refill
• Quantity: 1
...
- **Month column:** `Mon-YYYY` (e.g. `Feb-2026`)
- Derive Month from Date automatically — captain should not need to specify month separately
...
| Image is unreadable | Send: "I can't read this image clearly. Can you type out the details? I need: item, amount, head of account, payment mode." |
...
- Date format: DD/MM/YYYY, Month format: Mon-YYYY (e.g. Feb-2026)
